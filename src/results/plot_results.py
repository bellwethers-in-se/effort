...
def list_communities():
    for var in ["bell", "tca", "tnb"]:
        ...
        files = glob(os.path.abspath(os.path.join(".", "cocomodata", var, "*.csv")))
        yield files
    ...
    for vars in list_communities():
        for var in vars:
            pd_list.update({var.split("/")[-1].split(".")[0]: DataFrame(
                sorted(read_csv(var)[["Name", "MMRE (Mean)"]].values, key=lambda x: x[0], reverse=True))})
        ...
        for idx, key in enumerate(keys):  # Populate 2-D array
            for i, val in enumerate(pd_list[key][1].values):
                if i < idx:
                    stats[i, idx] = val
                if i > idx:  # Ensure self values are set to zero
                    stats[i+1, idx] = val
        ...
        stats = stats.sort_values(by="Mean", axis=0, ascending=True, inplace=False)
